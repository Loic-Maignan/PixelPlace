    ...
    if interactive_loop:
        ...
        while (not is_interrupted) and igs.is_started():
            ...
            def nouvelle_image(image, taille_pixel):
                ...
                image_redimensionnee = image_recu.resize(
                ...
                image_id_tmp = image_canvas.create_image(
                    posx, posy, anchor="nw", image=image_canvas_image
                )
            ...
            def choisir_couleur(event):
                x = event.x
                y = event.y
                couleur_selectionnee = couleur_canvas.itemcget(
                    couleur_canvas.find_closest(x, y), "fill"
                )
            ...
            def obtenir_case_grille(event):
                ...
                print(
                    f"on a x : {x} on a taille case = {taille_case} donc on est colonne : {x // taille_case}"
                )
                ...
                message = (
                    f"Coordonnées: x={str(case_x)}, y={str(case_y)}, couleur={couleur}"
                )
                igs.service_call("Whiteboard", "chat", message, "")
            ...
            def zoom(event):
                ...
                image_canvas.delete(image_id)
                image_id_tmp = image_canvas.create_image(
                    2, 2, anchor="nw", image=image_canvas_image
                )
            ...
            def finir_drag(event):
                ...
                image_canvas.delete(image_id)
                image_id_tmp = image_canvas.create_image(
                    move_x + 2, move_y + 2, anchor="nw", image=image_canvas_image
                )
                image_id = image_id_tmp
                print(
                    f"Fin du drag - déplacement: x={move_x}, y={move_y}, position: {posx}, {posy}"
                )
            ...
            titre_label = tk.Label(
                window,
                text=titre_text,
                font=("Comic Sans MS", ajuster_taille(20, proportion_x)),
            )

            # Calcul pour centrer le titre horizontalement
            titre_width = ajuster_taille(
                12 * len(titre_text), proportion_x
            )  # Largeur estimée du texte
            titre_label.place(
                x=(wx - titre_width) / 2, y=ajuster_taille(1, proportion_y)
            )

            user_text = "Utilisateur: "
            user_label = tk.Label(
                window, text=user_text, font=("Arial", ajuster_taille(16, proportion_y))
            )
            user_label.place(
                x=ajuster_taille(1, proportion_x), y=ajuster_taille(10, proportion_y)
            )

            user_var = tk.StringVar(value=str(igs.agent_uuid()[-4:]))
            user_entry = tk.Entry(
                window,
                width=ajuster_taille(10, proportion_x),
                font=("Arial", ajuster_taille(16, proportion_y)),
                textvariable=user_var,
            )
            user_entry.place(
                x=ajuster_taille(110, proportion_x), y=ajuster_taille(10, proportion_y)
            )
            ...
            timer_label.place(
                x=ajuster_taille(1200, proportion_x), y=ajuster_taille(10, proportion_y)
            )
            ...
            timer_label2.place(
                x=ajuster_taille(1270, proportion_x), y=ajuster_taille(10, proportion_y)
            )
            ...
            image_canvas.place(
                x=ajuster_taille(
                    (wx - ajuster_taille(730, proportion_x)) / 2, proportion_x
                ),
                y=ajuster_taille(130, proportion_y),
            )
            ...
            image_redimensionnee = image_recu.resize(
                (
                    ajuster_taille(image_taille, proportion_x),
                    ajuster_taille(image_taille, proportion_y),
                ),
                Image.LANCZOS,
            )  # Ajusté à la taille de la fenêtre
            image_canvas_image = ImageTk.PhotoImage(image_redimensionnee)
            image_id = image_canvas.create_image(
                2, 2, anchor="nw", image=image_canvas_image
            )
            ...
            def survol_canvas(event):
                ...
                print(
                    f"Survol - Coordonnées: x={x}, y={y}, case x : {case_x}, case y : {case_y}"
                )
                print(
                    f"Survol - Coordonnées: x={x}, y={y}, case x : {x_var.get()}, case y : {y_var.get()}"
                )
                couleur = couleur_var.get()
                previsu_tmp = image_canvas.create_rectangle(
                    case_y * taille_case + (3 * scale_factor),
                    case_x * taille_case + (3 * scale_factor),
                    (case_y + 1) * taille_case + (3 * scale_factor),
                    (case_x + 1) * taille_case + (3 * scale_factor),
                    fill=couleur,
                    outline="",
                )
                previsu = previsu_tmp
            ...
            taille_carré = 40
            couleur_canvas = tk.Canvas(
                window,
                bg="white",
                width=ajuster_taille(len(couleurs) * taille_carré, proportion_x),
                height=ajuster_taille(taille_carré, proportion_y),
            )
            couleur_canvas.place(
                x=ajuster_taille(
                    (wx - ajuster_taille(len(couleurs) * taille_carré, proportion_x))
                    / 2,
                    proportion_x,
                ),
                y=ajuster_taille(820, proportion_y),
            )
            ...
            for i, couleur in enumerate(couleurs):
                ...
                couleur_canvas.create_rectangle(
                    x1, y1, x2, y2, fill=couleur, outline=""
                )
            ...
            # Coordonnées X et Y
            x_label = tk.Label(
                window, text="x :", font=("Arial", ajuster_taille(16, proportion_y))
            )
            x_label.place(
                x=ajuster_taille(300, proportion_x), y=ajuster_taille(10, proportion_y)
            )

            x_var = tk.StringVar(value="0")
            x_label_var = tk.Label(
                window,
                textvariable=x_var,
                font=("Arial", ajuster_taille(16, proportion_y)),
            )
            x_label_var.place(
                x=ajuster_taille(330, proportion_x), y=ajuster_taille(10, proportion_y)
            )

            y_label = tk.Label(
                window, text="y :", font=("Arial", ajuster_taille(16, proportion_y))
            )
            y_label.place(
                x=ajuster_taille(390, proportion_x), y=ajuster_taille(10, proportion_y)
            )

            y_var = tk.StringVar(value="0")
            y_label_var = tk.Label(
                window,
                textvariable=y_var,
                font=("Arial", ajuster_taille(16, proportion_y)),
            )
            y_label_var.place(
                x=ajuster_taille(420, proportion_x), y=ajuster_taille(10, proportion_y)
            )

            zoom_label = tk.Label(
                window, text="zoom :", font=("Arial", ajuster_taille(16, proportion_y))
            )
            zoom_label.place(
                x=ajuster_taille(480, proportion_x), y=ajuster_taille(10, proportion_y)
            )

            zoom_var = tk.StringVar(value="1.0")
            zoom_label_var = tk.Label(
                window,
                textvariable=zoom_var,
                font=("Arial", ajuster_taille(16, proportion_y)),
            )
            zoom_label_var.place(
                x=ajuster_taille(550, proportion_x), y=ajuster_taille(10, proportion_y)
            )

            croix_label = tk.Label(
                window,
                text="X",
                width=ajuster_taille(2, proportion_x),
                height=ajuster_taille(1, proportion_y),
                font=("Arial", ajuster_taille(16, proportion_y)),
            )
            croix_label.place(
                x=ajuster_taille(1510, proportion_x), y=ajuster_taille(0, proportion_y)
            )
            ...
            chat_text = tk.Text(
                window,
                wrap=tk.WORD,
                width=ajuster_taille(40, proportion_x),
                height=ajuster_taille(45, proportion_y),
            )
            chat_text.place(
                x=ajuster_taille(5, proportion_x), y=ajuster_taille(50, proportion_y)
            )
            chat_text.config(state="disabled")

            chat_entry = tk.Entry(
                window,
                width=ajuster_taille(27, proportion_x),
                font=("Arial", ajuster_taille(16, proportion_y)),
            )
            chat_entry.place(
                x=ajuster_taille(5, proportion_x), y=ajuster_taille(780, proportion_y)
            )
            ...
            croix_label.bind(
                "<Leave>", lambda event: croix_label.configure(bg=window.cget("bg"))
            )
            ...
            explain_label.place(
                x=ajuster_taille(1150, proportion_x), y=ajuster_taille(50, proportion_y)
            )
            ...
            def charger_image_url():
                url = simpledialog.askstring(
                    "URL",
                    "Entrez l'URL de l'image (doit être un lien direct .jpg, .png, etc.):",
                )
                if url:
                    try:
                        ...
                        messagebox.showerror(
                            "Erreur", f"Impossible de charger l'image: {str(e)}"
                        )

            # Créer les boutons
            btn_locale = tk.Button(
                window, text="Charger image locale", command=charger_image_locale
            )
            btn_locale.place(
                x=ajuster_taille(1150, proportion_x),
                y=ajuster_taille(790, proportion_y),
            )

            btn_url = tk.Button(
                window, text="Charger image URL", command=charger_image_url
            )
            btn_url.place(
                x=ajuster_taille(1300, proportion_x),
                y=ajuster_taille(790, proportion_y),
            )

            aide_canva = tk.Canvas(
                window,
                bg="white",
                width=ajuster_taille(350, proportion_x),
                height=ajuster_taille(350, proportion_y),
            )
            aide_canva.place(
                x=ajuster_taille(1150, proportion_x),
                y=ajuster_taille(430, proportion_y),
            )
